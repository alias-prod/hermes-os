...
} from "@/lib/engines/hermes/HermesEngine";
import type {
  AppStore,
  ArtifactStore,
  GatewayCommand,
...
export type { HermesThreadListItem, ModelChoice, SessionRow } from "@/types/gateway-responses";
...
export function useGateway({ onAuthFailed }: { onAuthFailed: () => void }) {
  ...
  const engineRef = useRef<HermesEngine | null>(null);
        ...
        onSessionChanged: (sessionKey) => {
          for (const listener of sessionChangedListenersRef.current) {
            try {
              ...
              console.warn("[hermes] onSessionChanged listener threw:", err);
            }
          }
        },
        ...
        onCronChanged: () => {
          ...
          cronRefreshTimerRef.current = window.setTimeout(() => {
            ...
            void cronRefreshFnRef.current?.().catch((err) => {
              console.warn("[hermes] cron refresh after event failed:", err);
            });
          }, 150);
        },
      },
  ...
  const fetchThreadList = useCallback(
    async (): Promise<HermesThreadListItem[]> => engineRef.current?.fetchThreadList() ?? [],
    [],
  );
  ...
  useEffect(() => {
    ...
    void engineRef.current
      ...
      .catch((err) => console.warn("[hermes] fetchGatewayCommands failed:", err));
