/**
 * Merges Hermes chat.history messages into UI-ready turns.
 *
 * A single assistant "turn" in Hermes history spans multiple messages:
  ...
  for (const m of raw) {
    ...
    if (m.role === "assistant") {
      ...
      if (isGatewayInjected) {
        ...
        // as a `gateway-injected` message tagged `hermesAbort` whenever a
        ...
        const isAbortSnapshot = Boolean(
          (m as unknown as { hermesAbort?: { aborted?: boolean } }).hermesAbort?.aborted,
        );
        if (isAbortSnapshot) {
          continue;
        }
    ...
    // System messages with `__hermes.kind === "compaction"` are markers
