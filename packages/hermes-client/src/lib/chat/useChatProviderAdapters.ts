...
import type { HermesThreadListItem } from "@/types/gateway-responses";
...
function toThreadRow(r: HermesThreadListItem): Thread {
  return {
    id: r.id,
    title: r.title,
    createdAt: r.createdAt,
    hermesKind: r.hermesKind,
    hermesAgentId: r.hermesAgentId,
  } as Thread;
}

interface Args {
  fetchThreadList: () => Promise<HermesThreadListItem[]>;
...
export function useChatProviderAdapters({
  ...
  const adaptedLoadThread = useCallback(
    async (threadId: string): Promise<Message[]> => {
      ...
      // resolveChatSessionKey expands to `agent:<id>:main:hermes-os`.
      ...
      if (uploadsStore) {
        ...
        try {
          ...
          console.warn("[hermes] uploads.list failed:", error);
        }
        ...
        // `agent:<id>:main:hermes-os` because `knownAgentIds` hadn't
          ...
          const fetched = await Promise.all(
            missingIds.map(async (id) => {
              try {
                ...
                console.warn("[hermes] uploads.get fallback failed:", error);
                return null;
              }
            }),
          );
