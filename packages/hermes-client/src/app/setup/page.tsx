...
export default function SetupPage() {
  ...
  useEffect(() => {
    ...
    const gateway = params.get("apiBaseUrl") ?? params.get("gateway");
    const token = params.get("apiKey") ?? params.get("token");

    if (!gateway) {
      setStatus("error");
      setErrorMsg("Missing Hermes API base URL in the setup link.");
      return;
    }
  ...
  return (
    <div className="flex min-h-full items-center justify-center p-ml">
      <p className="text-sm text-text-neutral-tertiary">Configuring Hermes API connection...</p>
    </div>
  );
}
