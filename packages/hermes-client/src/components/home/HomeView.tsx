...
export function primeComposer(text: string, submit = false) {
  window.dispatchEvent(new CustomEvent("hermes-os:prime-composer", { detail: { text, submit } }));
}
...
/** Group `HermesThread[]` by agent — mirrors sidebar's buildAgentGroups. */
function buildAgents(threads: HermesThread[]): AgentCardData[] {
  const map = new Map<string, { id: string; name: string; threadCount: number }>();
  for (const t of threads) {
    const id = t.hermesAgentId ?? t.id;
    const existing = map.get(id);
    if (!existing) {
      map.set(id, { id, name: t.hermesKind === "main" ? t.title : id, threadCount: 1 });
    } else {
      existing.threadCount += 1;
      if (t.hermesKind === "main") existing.name = t.title;
    }
  }
...
function cronOwnerLabel(job: CronJobRecord, threads: Thread[]): string {
  ...
  const mainThread = agentPart
    ? threads.find(
        (t) =>
          // @ts-expect-error hermes-augmented thread fields
          (t.hermesAgentId ?? t.id) === agentPart && t.hermesKind === "main",
      )
    : undefined;
...
export function HomeView({
  ...
  const agents = useMemo(() => buildAgents(threads as HermesThread[]), [threads]);
  ...
  const openAgent = (agent: AgentCardData) => {
    const main = (threads as HermesThread[]).find(
      (t) => (t.hermesAgentId ?? t.id) === agent.id && t.hermesKind === "main",
    );
