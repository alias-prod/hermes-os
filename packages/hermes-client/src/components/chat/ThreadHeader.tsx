...
export function ThreadHeader({
  ...
  const currentAgentId = currentThread.hermesAgentId ?? currentThread.id;
  ...
  for (const t of allThreads) {
    ...
    if (t.hermesKind === "main") agentNameMap.set(aid, t.title);
  }
  const allAgents = [...agentNameMap.entries()].map(([id, name]) => ({ id, name }));
  const sessions = allThreads.filter((t) => (t.hermesAgentId ?? t.id) === currentAgentId);

  const findMain = (agentId: string): HermesThread | undefined =>
    allThreads.find((t) => (t.hermesAgentId ?? t.id) === agentId && t.hermesKind === "main") ??
    allThreads.find((t) => (t.hermesAgentId ?? t.id) === agentId);
