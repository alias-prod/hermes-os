...
import { HermesThreadContainer } from "@/components/layout/HermesThreadContainer";
...
import type { CompactSessionResult } from "@/lib/engines/hermes/HermesEngine";
import type {
  AppRecord,
  AppStore,
  AppSummary,
...
export function ThreadArea({
   ...
   *
   * Resolution order:
   *  1. Parse `agent:<id>:<slot>:hermes-os` directly from the route key
  ...
  const activeAgentId = useMemo(() => {
    ...
    const t = (allThreadsRaw as unknown as HermesThread[]).find((x) => x.id === selectedThreadId);
    return t?.hermesAgentId ?? t?.id ?? null;
  }, [allThreadsRaw, knownAgentIds, selectedThreadId]);

  /** Display name for the agent owning the current thread (the `hermesKind:
   *  "main"` thread's title). Used by the empty-chat welcome screen. */
  const activeAgentName = useMemo(() => {
    if (!activeAgentId) return undefined;
    const main = (allThreadsRaw as unknown as HermesThread[]).find(
      (t) => (t.hermesAgentId ?? t.id) === activeAgentId && t.hermesKind === "main",
    );
    return main?.title;
  ...
  useEffect(() => {
    ...
    const refreshLoop = async () => {
      try {
        ...
        console.warn("[hermes] durable refresh failed:", error);
      }
      if (cancelled) return;
  ...
  const addFiles = useCallback(
    async (files: File[]) => {
      ...
      // after Hermes's 2-minute media TTL expires. Use the resolved session
    ...
    const handler = (changedKey: string) => {
      ...
      pending = setTimeout(() => {
        ...
        void loadThread(selectedThreadId)
          ...
          .catch((err) => console.warn("[hermes] session-changed reload failed:", err));
      }, 400);
    };
  ...
  return (
    <UploadsProvider store={uploads} sessionKey={sessionKey} seeds={uploadSeeds}>
      <div className="flex h-full min-w-0 flex-1 overflow-hidden bg-background dark:bg-sunk">
        ...
        <HermesThreadContainer className="hermes-os-thread-container min-w-0 flex-1">
          <ThreadHeader
            allThreads={allThreadsRaw as unknown as HermesThread[]}
              ...
              <SessionComposer
                ...
                onStop={() => {
                  ...
                  void abort(selectedThreadId).catch((err) => {
                    console.warn("[hermes] chat.abort failed:", err);
                  });
                }}
                ...
                {...(() => {
                  // Map hermes's SessionRow fields onto the ring's
        ...
        </HermesThreadContainer>

        <ThreadArtifactPanels
          appList={appList}
          artifactList={artifactList}
          paneUploads={paneUploads}
          threads={allThreadsRaw as unknown as HermesThread[]}
          apps={apps}
          artifacts={artifacts}
          uploads={uploads}
