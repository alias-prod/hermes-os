    ...
    segment.type === "openui" ? (
      <Renderer
        ...
        onError={(errors) => {
          ...
          if (errors.length > 0) {
            console.warn("[hermes:assistant-render]", { messageId: message.id, errors });
          }
        }}
      />
    ) : isGatewayInjected ? (
      <div
        key={`text-${i}`}
        className="hermes-os-assistant-markdown mr-auto w-full max-w-3xl border-l-2 border-border-default/40 bg-transparent pl-m"
      >
        {i === 0 ? (
      ...
      <div
        key={`text-${i}`}
        className="hermes-os-assistant-markdown mr-auto w-full max-w-3xl rounded-4xl border border-border-default/40 bg-background px-ml py-m shadow-sm dark:border-border-default/20 dark:bg-sunk-deep"
      >
        <div className={ASSISTANT_MARKDOWN_CLASSES}>
          <ReactMarkdown remarkPlugins={[remarkGfm, remarkBreaks]}>{segment.content}</ReactMarkdown>
        </div>
