...
export function CronDetailTray({
  ...
  return (
    <div
      className={`relative flex h-full shrink-0 flex-col border-l border-border-default/50 bg-background dark:border-border-default/16 ${mobile ? "" : "hermes-slide-in-right"}`}
      style={{ width }}
    >
      {mobile ? null : (
        <TopBar
        ...
        {tab === "details" ? (
          <DetailsPanel
            ...
            onOpenAgent={() => {
              ...
              if (!agentId) return;
              const main = (
                threads as { id: string; hermesAgentId?: string; hermesKind?: string }[]
              ).find((t) => (t.hermesAgentId ?? t.id) === agentId && t.hermesKind === "main");
