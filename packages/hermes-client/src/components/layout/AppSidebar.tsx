...
function buildAgentGroups(threads: HermesThread[]): AgentGroup[] {
  const map = new Map<string, AgentGroup>();
  for (const t of threads) {
    const aid = t.hermesAgentId ?? t.id;
    let g = map.get(aid);
    if (!g) {
      g = { agentId: aid, headerTitle: aid, threads: [] };
      map.set(aid, g);
    }
    if (t.hermesKind === "main") g.headerTitle = t.title;
    g.threads.push(t);
  }
  return [...map.values()];
}
...
export function AppSidebar({
  ...
  const { threads, isLoadingThreads, selectedThreadId, loadThreads, selectThread } =
    useThreadList();
  const threadsCast = threads as HermesThread[];
  ...
  const displayThreads = useMemo<HermesThread[]>(
    () =>
      threadsCast
  ...
  useEffect(() => {
    if (route?.view === "chat" && activeChatId) {
      ...
      if (match?.hermesAgentId) {
        setExpandedAgent(match.hermesAgentId);
        return;
      }
    }
  ...
  const recentGroups = useMemo(() => {
    const latestCreatedAt = (threads: HermesThread[]): string =>
  ...
  return (
    ...
    >
      ...
      >
        <Logo name="Hermes" suffix="OS" collapsed={nc} />
        <IconButton
            ...
            <div className="pb-3xs">
              {isLoadingThreads && groups.length === 0 ? (
                ...
                recentGroups.slice(0, 5).map((g) => {
                  ...
                  const main = sessions.find((t) => t.hermesKind === "main");
                  const extras = sessions.filter((t) => t.hermesKind !== "main");
                  ...
                  return (
                    ...
                    >
                      {ordered.map((t) => {
                        ...
                        const isExtra = t.hermesKind !== "main";
