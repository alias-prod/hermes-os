...
function buildAgents(threads: HermesThread[]): AgentViewModel[] {
  const map = new Map<
    string,
  ...
  for (const t of threads) {
    ...
    if (!existing) {
      map.set(id, {
        id,
        name: t.hermesKind === "main" ? t.title : id,
        latestThreadId: t.id,
...
export function MobileAgentsView({ threads, onOpenThread }: AgentsViewProps) {
  ...
  return (
    <div className="hermes-fade-in h-full flex-1 overflow-y-auto bg-background p-ml">
