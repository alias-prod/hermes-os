...
import { openHermesAdapter } from "@/lib/chat/openHermesAdapter";
...
function ChatAppInner({
  ...
  useEffect(() => {
    ...
    trackers.set(sessionKey, {
      ...
      agentId: currentThread?.hermesAgentId ?? selectedThreadId,
   ...
   *   2. agent's `hermesKind === "main"` thread, if the originating one is gone
  ...
  const resolveRefineThreadId = useCallback(
    async (sessionKey: string | undefined, agentId: string | undefined) => {
      ...
      if (agentId) {
        const main = (threads as unknown as HermesThread[]).find(
          (t) => (t.hermesAgentId ?? t.id) === agentId && t.hermesKind === "main",
        );
  ...
  const contextValue: ChatAppContextValue = useMemo(
    () => ({
      threads: threads as unknown as HermesThread[],
      appList,
      artifactList,
      notifications,
      cronJobs,
  ...
  if (isMobile) {
    return (
      <ChatAppProvider value={contextValue}>
        <Shell.Container agentName="Hermes" logoUrl={LOGO_URL}>
          <MobileShell
            route={route}
  ...
  return (
    <ThemeProvider mode={themeMode}>
      <ChatProvider
        ...
        streamProtocol={openHermesAdapter()}
      >
        <ChatAppInner
          connectionState={connectionState}
